        ...
        try {
          ...
          this.parsedData = json.map((row) => {
            ...
            return {
              ...
              program_name: programMap[row.Program.trim()] || row.Program.trim(),
    ...
    async submitUpload() {
      ...
      try {
        ...
        const existingSet = new Set(
          existingCourses
            .map((course) => {
              ...
              }-${course.curriculum.program.institute?.institute_code}-${start}-${end}-${
                course.course_level
        ...
        const uniqueInstitutes = [
          ...new Map(this.parsedData.map((row) => [row.institute_code, row])).values(),
        ];
        ...
        const coursesPayload = this.parsedData.map((row) => {
          ...
          const key = `${normalize(row.course_code)}-${row.program_code}-${
            row.institute_code
          }-${row.curriculum_start_year}-${row.curriculum_end_year}-${row.course_level}-${
            row.course_semester
          }`;
          ...
          return {
            ...
            payload: {
              ...
              program_id: programMap.get(`${row.institute_code}-${row.program_code}`),
