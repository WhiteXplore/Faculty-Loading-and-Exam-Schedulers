...
export default {
  ...
  mounted() {
    ...
    this.timeStart = event.timeStart || "09:30";
    this.timeEnd = event.timeEnd || "10:30";
    this.isAllDay = event.isAllDay || false;
    this.selectedProgramId = event.program_id?.toString() || "";

    // Find institute of the selected program
    const selectedProgram = this.dataStore.programs.find(
      (p) => p.program_id === parseInt(this.selectedProgramId)
    );
    this.selectedInstituteId = selectedProgram?.institute_id?.toString() || "";
    this.filterPrograms();
  },
  methods: {
    filterPrograms() {
    ...
    updateEvent() {
      ...
      this.$emit("update", {
        id: this.eventToEdit.id, // retain ID
