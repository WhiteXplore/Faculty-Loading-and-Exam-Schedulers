      ...
      <form @submit.prevent="submitData" class="w-auto bg-white text-[13px] rounded-[16px] shadow-lg "
        ...
        <div class="p-5 w-[35vw] space-y-5">
          ...
          <div class="flex flex-col space-y-2 w-full relative">
            ...
            <input v-model="searchCurriculumQuery" type="text" placeholder="Search curriculum..."
              class="px-3 py-3 border w-full border-gray-600 rounded-md text-md text-gray-800"
              @focus="showCurriculumDropdown = true" />
            <div v-if="showCurriculumDropdown && filteredCurriculum.length"
              ...
              <div v-for="curriculum in filteredCurriculum" :key="curriculum.curriculum_id"
                class="px-3 py-2 hover:bg-gray-100 cursor-pointer" @mousedown="selectcurriculum(curriculum)">
          ...
          <div class="w-full space-y-2">
            ...
            <input v-model="form.course_code" type="text" id="course_code" required
          ...
          <!-- Description -->
          <div class="w-full space-y-2">
            <label for="course_description" class="font-bold">Course Description:</label>
            <textarea v-model="form.course_description" id="course_description" required
              class="w-full border px-3 py-3 border-gray-600 rounded-md text-md text-gray-800"
          ...
          <div class="w-full flex gap-3">
            <div class="w-full space-y-2">
              ...
              <select v-model="form.course_level" required class="w-full border px-2 py-3 border-gray-600 rounded-md">
            ...
            <div class="w-full space-y-2">
              ...
              <select v-model="form.course_semester" required
          ...
          <div class="w-full flex gap-3">
            <div class="w-full space-y-2">
              ...
              <input v-model="form.course_lec" type="number" required
                class="w-full border px-3 py-3 border-gray-600 rounded-md" />
            </div>
            <div class="w-full space-y-2">
              <label class="font-bold">Laboratory (Hours):</label>
              <input v-model="form.course_lab" type="number" required
                class="w-full border px-3 py-3 border-gray-600 rounded-md" />
            </div>
          </div>
          ...
          <div class="flex flex-col space-y-2 relative">
            ...
            <input :value="form.course_requisite.join(', ')" @input="searchRequisiteQuery = $event.target.value"
              @focus="showRequisiteDropdown = true" type="text" placeholder="Search course prerequisite..."
              class="px-3 py-3 border w-full border-gray-600 rounded-md" />
            <div v-if="showRequisiteDropdown && filteredCourse.length"
              class="absolute top-[75px] w-full bg-white border border-gray-300 rounded-md max-h-40 overflow-y-auto z-10"
              @mouseleave="showRequisiteDropdown = false">
              <div v-for="course in filteredCourse" :key="course.course_id"
                class="px-3 py-2 hover:bg-gray-100 cursor-pointer" @mousedown="selectcourse(course)">
                {{ course.course_code }} - {{ course.course_description }}
              </div>
            </div>
            <div class="flex flex-wrap gap-2 mt-2">
              <span v-for="code in form.course_requisite" :key="code"
          ...
          <div class="flex flex-col space-y-2 relative">
            ...
            <input :value="form.course_tags.join(', ')" @input="searchTagQuery = $event.target.value"
              @focus="showTagDropdown = true" type="text" placeholder="Select tags..."
              class="px-3 py-3 border w-full border-gray-600 rounded-md" />
            <div v-if="showTagDropdown && filteredTags.length"
              class="absolute top-[75px] w-full bg-white border border-gray-300 rounded-md max-h-40 overflow-y-auto z-10"
              @mouseleave="showTagDropdown = false">
              <div v-for="tag in filteredTags" :key="tag" class="px-3 py-2 hover:bg-gray-100 cursor-pointer"
                @mousedown="selectTag(tag)">
                {{ tag }}
              </div>
            </div>
            <div class="flex flex-wrap gap-2 mt-2">
              <span v-for="tag in form.course_tags" :key="tag"
                class="bg-blue-100 text-blue-800 px-2 py-1 rounded-full text-xs cursor-pointer" @click="removeTag(tag)">
                {{ tag }} ✕
              </span>
            </div>
          </div>
